...
## Stack
Next.js 16 (App Router, `src/`), React 19, TypeScript, Tailwind CSS v4, next-intl (EN/TH), next-themes (dark/light), GSAP + ScrollTrigger, motion, lucide-react. Package manager: **bun**

## Structure
```
src/app/[locale]/       routes (page.tsx สะอาด ไม่มี business logic)
...
src/constants/          ข้อมูล portfolio (แทนที่ด้วยข้อมูลจริงได้เลย)
src/services/           portfolio-service.ts เป็นชั้นกลางก่อนถึง UI
src/i18n/               routing / request / navigation ของ next-intl
...
## Next.js 16 notes
Middleware เปลี่ยนชื่อเป็น **Proxy** (`src/proxy.ts`) · `params`/`searchParams` เป็น Promise ต้อง `await` · เอกสารจริงอยู่ที่ `node_modules/next/dist/docs/`
